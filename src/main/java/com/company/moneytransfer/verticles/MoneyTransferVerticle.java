package com.company.moneytransfer.verticles;

import com.company.moneytransfer.model.Account;
import com.company.moneytransfer.model.MoneyTransfer;
import com.company.moneytransfer.service.AccountService;
import com.company.moneytransfer.service.MoneyTransferService;
import com.company.moneytransfer.util.Constants;
import com.company.moneytransfer.util.DBUtil;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule;
import com.company.moneytransfer.service.UserService;
...
public class MoneyTransferVerticle extends AbstractVerticle {
	...
	private Future<Void> startHttpServer(Router router) {
		...
		router.get("/api/users").produces("application/json").consumes("application/json").handler(this::getUsers);
		router.get("/api/users/:id").produces("application/json").consumes("application/json").handler(this::getUserById);
		router.get("/api/accounts/:userid").produces("application/json").consumes("application/json").handler(this::getAccountsByUserId);
		router.post("/api/transfer").produces("application/json").consumes("application/json").handler(this::transfer);
		router.get("/api/transfer/:accountid").produces("application/json").consumes("application/json").handler(this::getTranfersForAccount);
    ...
    private void getUsers(RoutingContext routingContext) {
    	...
    	resp.onComplete( resultHandler( routingContext, res -> {
    ...
    private void getUserById(RoutingContext routingContext) {
		...
		Promise<JsonObject> resp = UserService.getInstance().findUserById( conn, userId );
		Future<JsonObject> resp2 = resp.future();
		resp2.onComplete( resultHandler( routingContext, res -> {
    ...
    private void transfer(RoutingContext routingContext) {
    	...
    	try {
			...
			final MoneyTransfer moneyTransfer = Json.decodeValue( body, MoneyTransfer.class);
        	...
        	fromAccountQueryResponse.onComplete( resultHandler( routingContext, resFromAccount -> {
        		...
        		if( resFromAccount.getString("status").equals("ok") ) {
                	...
                	toAccountQueryResponse.onComplete( resultHandler( routingContext, resToAccount -> {
                		...
                		if( resToAccount.getString("status").equals("ok") ) {
                			...
                			CompositeFuture.all(fromAccountQueryResponse,toAccountQueryResponse).onComplete( r-> {
                        		...
                        		if( fromAccountQueryResponse.succeeded() && fromAccountQueryResponse.isComplete() ) {
                            		...
                            		if( toAccountQueryResponse.succeeded() && toAccountQueryResponse.isComplete() ) {
                                    	...
                                    	Future<JsonObject> resp = MoneyTransferService.getInstance().transfer( conn, accounts[0], accounts[1],
                    	   				...
                    	   				resp.onComplete( resultHandler( routingContext, resTransfer -> {
                    	            		...
                    	            		if( resp.succeeded() && resp.isComplete() && resTransfer.getString("status").equals("ok") ) {
                    							...
                    							resultFromAccount.onComplete( resultHandler( routingContext, resTransferQueryFromAccountValue -> {
                    								...
                    								if( resultFromAccount.succeeded() && resultFromAccount.isComplete() 
                        								...
                        								resultToAccount.onComplete( resultHandler( routingContext, resTransferQueryToAccountValue -> {
